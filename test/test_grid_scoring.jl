...
@testset "Grid Scoring" begin
    ...
    @testset "deceptive minima" begin
        ...
        @testset "far minimum with similar value is deceptive" begin
            ...
            deceptive =
                _detect_deceptive_minima(minima_pts, minima_vals, p_true, bounds, 2.0, 0.1)
    ...
    @testset "interestingness score" begin
        @testset "boring landscape (single minimum, full basin)" begin
            gs = GridScoreResult(
                ...
                [[0.0, 1.0, 2.0, 3.0, 4.0], [0.0, 1.0, 2.0, 3.0, 4.0]],   # grid_axes
                [5, 5],                           # grid_size
                1,                                # n_local_minima
        ...
        @testset "basin depth ratio discounted by multimodal context" begin
            ...
            make_gs =
                (n_min, dr) -> GridScoreResult(
                    ...
                    1.0,
                    0,
                    Vector{Float64}[],   # basin_fraction=1 ⇒ basin term=0
                    25,
                    25,
                    1.0,
                    zeros(2),
            ...
            @test interestingness_score(make_gs(5, 1.0)) ≈ 0.12 + 0.20 * (4 / 14) atol =
                1e-9
        end
        ...
        @testset "score is bounded [0, 1]" begin
            ...
            gs = GridScoreResult(
                ...
                100.0,
                100.0,
                1.0,     # curv, depth, cond, rugged, plateau (all clamp to 1)
            )
            score = interestingness_score(gs)
            @test 0.0 <= score <= 1.0
        end
    ...
    @testset "print_grid_score" begin
        gs = GridScoreResult(
            ...
            2.0,
            0.5,
            0.3,             # curv, depth, cond, rugged, plateau
        )
        buf = IOBuffer()
