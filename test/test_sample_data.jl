...
@testset "sample_data" begin
    ...
    @testset "noise injection changes data" begin
        ...
        data_noisy = sample_data(problem, model, outputs, time_interval, p_true, ic, 15;
            inject_noise=true, mean_noise=0.0, stddev_noise=0.1)
    ...
    @testset "custom solver" begin
        data = sample_data(problem, model, outputs, time_interval, p_true, ic, 15;
            solver=Tsit5(), abstol=1e-6, reltol=1e-6)
        @test length(data["t"]) == 15
    ...
    @testset "uneven sampling times" begin
        ...
        data = sample_data(problem, model, outputs, time_interval, p_true, ic,
            length(custom_times);
            uneven_sampling=true, uneven_sampling_times=custom_times)
        @test collect(data["t"]) ≈ custom_times
    end

    @testset "uneven sampling — length mismatch errors" begin
        @test_throws ErrorException sample_data(problem, model, outputs, time_interval,
            p_true, ic, 10;
            uneven_sampling=true, uneven_sampling_times=[0.0, 1.0])
    end

    @testset "uneven sampling — empty times errors" begin
        @test_throws ErrorException sample_data(problem, model, outputs, time_interval,
            p_true, ic, 5;
            uneven_sampling=true, uneven_sampling_times=Float64[])
    end
end
