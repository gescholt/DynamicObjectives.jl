...
@testset "Experiment Utilities" begin
    ...
    @testset "CandidateResult construction" begin
        r = CandidateResult(
            "test", [1.0, 2.0], 6, 3, 0.001, 0.01, 0.05, 0.1, 0.8, 0.95
        )
        @test r.name == "test"
    ...
    @testset "print_candidate_summary_table" begin
        ...
        try
            best = print_candidate_summary_table(results, entries;
                title="Test", radius=0.1, degree_range=4:2:6)
            @test length(best) == 2
    ...
    @testset "print_candidate_summary_table — NaN values" begin
        results = [
            CandidateResult("NaN_model", [1.0], 4, 0, NaN, NaN, 0.0, NaN, NaN, NaN),
        ]
        ...
        try
            best = print_candidate_summary_table(results, entries;
                title="NaN Test", radius=0.1, degree_range=4:2:4)
