    ...
    for (name, fn) in MODEL_REGISTRY
        @testset "$name" begin
            ...
            if haskey(model_configs, name)
                ...
                err_fn = make_error_distance(model, outputs, cfg.ic, cfg.p, [0.0, 5.0], 10)
                val = err_fn(cfg.p)
                @test isfinite(val)
