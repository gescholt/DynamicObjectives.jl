...
@testset "Screening" begin
    ...
    @testset "is_trajectory_bounded — LV 2D" begin
        ...
        @testset "low threshold catches moderate amplitudes" begin
            result = is_trajectory_bounded(model, ic, [1.0, 0.5], [0.0, 10.0]; threshold=1.0)
        ...
        @testset "custom solver and tolerance" begin
            result = is_trajectory_bounded(
                model, ic, [1.0, 0.5], [0.0, 10.0];
                solver=Vern9(), abstol=1e-10, reltol=1e-10, numpoints=50
            )
            @test result.bounded == true
            @test result.solver_success == true
        end
    ...
    @testset "sweep_p_true — LV 2D grid" begin
        ...
        result = sweep_p_true(model, ic, bounds, [0.0, 10.0];
            n_candidates=20, sampling=:grid)
    ...
    @testset "sweep_p_true — input validation" begin
        ...
        @testset "invalid margin" begin
            @test_throws AssertionError sweep_p_true(model, ic, bounds, [0.0, 10.0]; margin=0.5)
            @test_throws AssertionError sweep_p_true(model, ic, bounds, [0.0, 10.0]; margin=-0.1)
        end

        @testset "invalid n_candidates" begin
            @test_throws AssertionError sweep_p_true(model, ic, bounds, [0.0, 10.0]; n_candidates=0)
        end
        ...
        @testset "unknown sampling strategy" begin
            @test_throws ErrorException sweep_p_true(model, ic, bounds, [0.0, 10.0];
                sampling=:sobol)
        end
    end
    ...
    @testset "sweep_p_true — performance" begin
        ...
        println("  Sweep performance: $(round(result.diagnostics.ms_per_candidate, digits=1)) ms/candidate (LV 2D, 50 candidates)")
    end
    ...
    @testset "is_trajectory_bounded — LV 4D Constrained" begin
        ...
        @testset "small perturbations are bounded at t=10" begin
            ...
            result = is_trajectory_bounded(model, ic, [0.01, 0.02, -0.01, 0.03], [0.0, 10.0])
            @test result.bounded == true
            @test result.solver_success == true
            @test isfinite(result.max_amplitude)
        end
        ...
        @testset "near-zero perturbations are bounded long-term" begin
            ...
            result = is_trajectory_bounded(model, ic, [0.001, 0.001, -0.001, 0.001], [0.0, 30.0])
            @test result.solver_success == true
            @test result.bounded == true
        end
    end
    ...
    @testset "is_trajectory_bounded — input validation" begin
        ...
        @testset "wrong parameter length" begin
            ...
            @test_throws AssertionError is_trajectory_bounded(model, ic, [1.0, 0.5, 0.3], [0.0, 10.0])
        end

        @testset "wrong IC length" begin
            @test_throws AssertionError is_trajectory_bounded(model, [0.5], p, [0.0, 10.0])
            @test_throws AssertionError is_trajectory_bounded(model, [0.5, 1.0, 0.3], p, [0.0, 10.0])
        end
        ...
        @testset "invalid threshold" begin
            @test_throws AssertionError is_trajectory_bounded(model, ic, p, [0.0, 10.0]; threshold=-1.0)
            @test_throws AssertionError is_trajectory_bounded(model, ic, p, [0.0, 10.0]; threshold=0.0)
        end
    end
...
@testset "Probe landscape" begin
    ...
    @testset "probe_landscape — LV 2D" begin
        ...
        result = probe_landscape(model, outputs, ic, p_true, bounds, [0.0, 10.0];
            n_probes=15)
    ...
    @testset "probe_landscape — sweep integration" begin
        ...
        result = probe_landscape(model, outputs, ic, p_candidate, bounds, [0.0, 10.0];
            n_probes=5)
    ...
    @testset "probe_landscape — multiple candidates ranking" begin
        ...
        for p in sweep.valid[1:min(5, length(sweep.valid))]
            result = probe_landscape(model, outputs, ic, p, bounds, [0.0, 10.0];
                n_probes=5)
            push!(probes, result)
        end
    ...
    @testset "probe_landscape — input validation" begin
        ...
        @testset "wrong bounds length" begin
            bad_bounds = [(0.5, 2.0)]
            @test_throws AssertionError probe_landscape(model, outputs, ic, p_true,
                bad_bounds, [0.0, 10.0])
        end

        @testset "wrong p_true length" begin
            @test_throws AssertionError probe_landscape(model, outputs, ic, [1.0],
                bounds, [0.0, 10.0])
        end

        @testset "invalid n_probes" begin
            @test_throws AssertionError probe_landscape(model, outputs, ic, p_true,
                bounds, [0.0, 10.0]; n_probes=0)
        end
    end
    ...
    @testset "probe_landscape — performance" begin
        ...
        println("  Probe performance: $(round(avg_ms, digits=1)) ms/probe (LV 2D, 5 probes)")
    end
...
@testset "Rank probes" begin
    ...
    function _fake_probe(; dr=1.0, var=100.0, n_finite=10, n_inf=0, obj_true=0.001)
        ProbeResult(
            obj_true,
    ...
    @testset "rank_probes — disabled filters" begin
        probes = [
            _fake_probe(dr=3.0, n_finite=1, n_inf=9, obj_true=999.0),
        ]
...
@testset "Screen and probe" begin
    ...
    @testset "screen_and_probe — LV 2D end-to-end" begin
        ...
        result = screen_and_probe(model, outputs, ic, bounds, [0.0, 10.0];
            n_candidates=20, n_probes=5)
    ...
    @testset "screen_and_probe — composability" begin
        ...
        result = screen_and_probe(model, outputs, ic, bounds, [0.0, 10.0];
            n_candidates=10, n_probes=3)

        # The orchestrator should produce consistent counts
        @test result.sweep.diagnostics.n_total == 10
        @test length(result.probes) == length(result.sweep.valid)
        n_total = length(result.ranking.ranked_indices) + length(result.ranking.filtered_out)
    ...
    @testset "screen_and_probe — input validation" begin
        ...
        @test_throws AssertionError screen_and_probe(
            model, outputs, ic, [(0.5, 2.0)], [0.0, 10.0]; n_candidates=5)
    end
    ...
    @testset "screen_and_probe — performance" begin
        ...
        # Warmup
        screen_and_probe(model, outputs, ic, bounds, [0.0, 10.0];
            n_candidates=5, n_probes=3)

        t_start = time()
        result = screen_and_probe(model, outputs, ic, bounds, [0.0, 10.0];
            n_candidates=30, n_probes=5)
        elapsed = (time() - t_start) * 1000

        @test elapsed < 5000.0  # 30 candidates + 30 probes should be well under 5s
        println("  screen_and_probe: $(round(elapsed, digits=0))ms (30 candidates, 5 probes)")
    end
