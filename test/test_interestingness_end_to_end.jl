...
import Dynamic_objectives: Tsit5
...
@testset "Interestingness end-to-end (bead bzf)" begin
    ...
    screening = screen_and_probe(
        model, outputs, ic, bounds, time_interval;
        n_candidates = 20,
        n_probes = 6,
        numpoints_screen = 10,
        numpoints_probe = 15,
    ...
    @testset "screen_and_probe produced a ranked result" begin
        @test isa(screening, Dynamic_objectives.ScreeningResult)
        @test !isempty(screening.sweep.valid)
        # Ranking length bounded by number of candidates that passed probe.
        @test length(screening.ranking.ranked_indices) <=
              length(screening.probes)
    end
    ...
    if isempty(screening.ranking.ranked_indices)
        ...
        grid_results = Dynamic_objectives.score_top_candidates(
            screening, model, outputs, ic, bounds, time_interval;
            top_n = top_n,
            points_per_dim = 4,
            numpoints = 10,
            solver = Tsit5(), abstol = 1e-4, reltol = 1e-4,
        ...
        @testset "interestingness_score lies in [0, 1]" begin
            for gs in successes
                s = Dynamic_objectives.interestingness_score(gs)
                @test isfinite(s)
                @test 0.0 <= s <= 1.0
            end
        end
