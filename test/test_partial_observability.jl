...
@testset "Partial observability (bead dds)" begin
    ...
    err_full = make_error_distance(
        model, outputs, ic, p_true, time_interval,
        10, L2_norm, sum, nothing;
        solver_kwargs...,
    )
    err_partial = make_partial_observability_distance(
        model, outputs, [1, 3], ic, p_true, time_interval,
        10, L2_norm, sum, nothing;
        solver_kwargs...,
    )
    ...
    @testset "Index validation" begin
        @test_throws ArgumentError make_partial_observability_distance(
            model, outputs, Int[], ic, p_true, time_interval,
        )
        @test_throws ArgumentError make_partial_observability_distance(
            model, outputs, [0, 1], ic, p_true, time_interval,
        )
        @test_throws ArgumentError make_partial_observability_distance(
            model, outputs, [1, 5], ic, p_true, time_interval,
        )
    end
end
