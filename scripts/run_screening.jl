...
#
# Usage:
#   julia --project=profiles/dev pkg/Dynamic_objectives/scripts/run_screening.jl CONFIG.toml
#
# The config file must contain a [screening] section with at least:
#   - model_factory   string key into Dynamic_objectives.MODEL_REGISTRY
#   - ic              [x0_1, x0_2, ...]
#   - bounds          [[lo, hi], ...]
...
function _resolve_model_factory(name::String)
    haskey(Dynamic_objectives.MODEL_REGISTRY, name) || error(
        "model_factory '$name' not registered in Dynamic_objectives.MODEL_REGISTRY. " *
        "Known factories: $(join(sort(collect(keys(Dynamic_objectives.MODEL_REGISTRY))), \", \"))",
    )
    return Dynamic_objectives.MODEL_REGISTRY[name]
end
...
function main(config_path::String)
    ...
    @printf("  %-4s  %-40s  %14s  %12s  %s\n", "Rank", "p_true", "dynamic_range", "variance", "Selected")
    println("  " * "-" ^ 90)
    for i in 1:n_show
        idx = ranked[i]
    ...
    @printf("  Pass rate: %.1f%% (%d/%d)\n",
            100.0 * sweep.pass_rate, sweep.diagnostics.n_valid, sweep.diagnostics.n_total)
...
if length(ARGS) == 0
    println(stderr, "Usage: julia --project=profiles/dev pkg/Dynamic_objectives/scripts/run_screening.jl CONFIG.toml")
    exit(1)
end
