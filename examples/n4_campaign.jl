...
function generate_model_report(config, result, timing, output_dir)
    io = IOBuffer()

    println(io, "# Model Report: $(config.name)")
    println(io)
    ...
    println(io, "| Dimension | $(length(config.p_true)) parameters |")
    ...
    println(io, "| Max Time | $(MAX_TIME)s |")
    ...
    if isfinite(result.recovery_error)
        println(io, "| Recovery Error | $(@sprintf("%.6e", result.recovery_error)) |")
    end
    if isfinite(result.objective_value)
        println(io, "| Best Objective | $(@sprintf("%.6e", result.objective_value)) |")
    end

    println(io, "| Critical Points | $(result.critical_points) |")
    println(io, "| Best Degree | $(something(result.best_degree, "-")) |")
    println(io, "| Success Rate | $(round(result.success_rate * 100, digits=1))% |")
    println(io)

    # Timing
    println(io, "## Timing")
    println(io)
    println(io, "| Stage | Duration |")
    println(io, "| --- | ---: |")
    if timing.stage1_time !== nothing
        println(io, "| Stage 1 (globtim) | $(round(timing.stage1_time, digits=2))s |")
    end
    if timing.stage2_time !== nothing
        println(io, "| Stage 2 (refinement) | $(round(timing.stage2_time, digits=2))s |")
    end
    println(io, "| **Total** | **$(round(timing.total_time, digits=2))s** |")
    println(io)
    ...
    println(io, "---")
    println(io, "*Report generated by Dynamic_objectives n4_campaign.jl*")
...
function test_model_with_timing(config, output_dir)
    ...
    try
        ...
        generate_model_report(config, result_tuple, timing, model_output_dir)

        return result_tuple, timing
        ...
        # Generate per-model report (even for failures)
        generate_model_report(config, result_tuple, timing, model_output_dir)
