# Run with: julia --project=profiles/dev --threads=auto pkg/Dynamic_objectives/examples/compare_3d_models.jl
#
# Compare 2 candidate 3D ODE models for interesting level set landscapes.
#
# Existing 3D catalogue entries (LV3D, FHN3D) produce flat/hyperplanar level sets
# because parameters appear mostly linearly. This script screens 2 models with
# genuinely nonlinear parameter coupling and compares landscape complexity metrics:
#
#   1. Rössler (a, b, c)  — z*(x-c) nonlinearity, rugged/chaotic
#   2. Goodwin 3D (k1, k2, k4) — Hill function K^n/(K^n+x3^n), structured multimodal
#
# Pipeline per model:
#   1. screen_and_probe(50 candidates, 5 probes) — lightweight discovery
#   2. score_landscape_grid(points_per_dim=15) — grid-based metrics on top candidate
...
const MODELS = [
    ModelSpec(
        ...
        [1.0, 1.0, 0.0],                                          # standard IC
        [(0.05, 0.5), (0.05, 0.5), (3.0, 9.0)],                   # a, b, c
        [0.0, 30.0],                                                # needs longer for oscillation
    ...
    ModelSpec(
        ...
        [0.5, 0.5, 0.5],                                          # moderate initial concentrations
        [(0.1, 2.0), (0.1, 2.0), (0.1, 2.0)],                     # k1, k2, k4
        [0.0, 30.0],                                                # oscillatory dynamics
        AutoTsit5(Rosenbrock23()),
        ["k1", "k2", "k4"],
    ),
]
...
println("=" ^ 100)
println("  3D Model Comparison: Rössler vs Goodwin3D")
...
@printf(
    "  %-12s  %9s  %8s  %10s  %8s  %8s  %10s  %10s  %7s  %6s  %6s  %6s  %12s\n",
    "Model",
    "pass_rate",
    "%finite",
    "dyn_range",
...
for r in results
    if r.grid_score === nothing
        @printf(
            "  %-12s  %8.1f%%  %8s  %10s  %8s  %8s  %10s  %10s  %7s  %6s  %6s  %6s  %12s\n",
            r.spec.name,
            100.0 * r.pass_rate,
            "—",
            "—",
        ...
        gs = r.grid_score
        @printf(
            "  %-12s  %8.1f%%  %7.1f%%  %10.2f  %8d  %7.1f%%  %10d  %10.3f  %7.3f  %6.1f  %6.3f  %6.2f  %12.3f\n",
            r.spec.name,
            100.0 * r.pass_rate,
            100.0 * gs.n_finite / gs.n_total,
            gs.dynamic_range,
...
for r in results
    ...
    model, params, states, outputs = spec.define_fn()
    cat_path =
        joinpath(REPO_ROOT, "globtim_results", "$(lowercase(spec.name))_catalogue.jsonl")
    ...
    entries = screening_to_catalogue(
        ...
        description = "3D model comparison: $(spec.name) ($(join(spec.param_names, ", "))). " *
                      "Solver: AutoTsit5(Rosenbrock23()). Dual-output screening.",
    )
...
println("=" ^ 100)
println(
    "  DONE — Run the winner in level_set_animation_3d.jl to visually confirm curvature",
)
println("=" ^ 100)
println()
