#!/usr/bin/env julia
"""
Integration Template: Pipeline Convenience Wrapper

This template demonstrates a BALANCED approach between simple and advanced:
- More convenient than manual 2-stage setup
- More control than the simple wrapper
...
PATTERN:
- Uses ExperimentParams for configuration
- Single convenience call that runs both stages
- Returns both raw and refined results
- Suitable for batch processing

ARCHITECTURE:
┌──────────────────────────────────────────────────────────────┐
│ Convenience wrapper (run_globtim_pipeline)                  │
│   └─> Stage 1: globtimcore (raw critical points)            │
│   └─> Stage 2: globtimpostprocessing (refinement)           │
│   Returns: (raw_result, refined_result)                     │
└──────────────────────────────────────────────────────────────┘

NOTE: As of Phase 2 migration, run_globtim_pipeline() needs updating.
      This template shows the INTENDED pattern.
"""
...
if !isdefined(Main, :ExperimentCLI)
    globtimcore_path = joinpath(dirname(dirname(@__DIR__)), "..", "globtimcore")
...
# When run_globtim_pipeline() is updated, use this instead:
...
println("  Convergence rate:      $(round(100*refined_result[:n_converged]/refined_result[:n_raw], digits=1))%")
...
println("  :degree_results      = Array of DegreeResult ($(length(raw_result[:degree_results])) entries)")
...
println("  :refined_points      = Array{Vector{Float64}} ($(length(refined_result[:refined_points])) entries)")
println("  :refined_values      = Array{Float64} ($(length(refined_result[:refined_values])) entries)")
