...
- Stage 1 (globtimcore): Polynomial approximation + HomotopyContinuation
...
#===============================================================================
STEP 2: Configure Stage 1 (globtimcore)
===============================================================================#
...
config = ExperimentParams(
    ...
    # Feature toggles (Phase 2: these are disabled in globtimcore)
...
println("\n" * "="^80)
println("Stage 1: Finding raw critical points with globtimcore")
println("="^80)
...
result = run_standard_experiment(
    ...
    problem_params = nothing,           # Not needed for Dynamic_objectives
...
for deg_result in result[:degree_results]
    ...
    println("    $status_symbol Degree $(deg_result.degree): $(deg_result.n_critical_points) points, best=$(round(deg_result.best_objective, digits=6))")
end
...
for deg_result in result[:degree_results]
    if deg_result.status == "success"
        csv_path = joinpath(output_dir, "critical_points_refined_deg_$(deg_result.degree).csv")
        if isfile(csv_path)
            println("  ✓ $csv_path")
        end
    end
...
#   - problem_params: Additional params (often nothing)
#   - domain_bounds: [(min, max), ...] for each dimension
