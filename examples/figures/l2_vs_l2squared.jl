...
using DynamicObjectives: define_lotka_volterra_2D_model
using CairoMakie

const ASSET_DIR = abspath(joinpath(@__DIR__, "..", "..", "docs", "src", "assets"))
isdir(ASSET_DIR) || mkpath(ASSET_DIR)

println("Building L2 vs L2² figure (LV2D_paper config) …")

model, _params, _states, outputs = define_lotka_volterra_2D_model()
ic            = [0.3, 0.6]
p_true        = [0.2, 0.4]
...
Label(fig[0, 1:4],
    "LV2D parameter-estimation objective near p* = $(p_true)";
...
ax_slice = Axis(fig[2, 1:4];
    title  = "1-D slice through p*  (b ≡ $(p_true[2]))",
