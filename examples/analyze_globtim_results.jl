...
# Try to load GlobtimPostProcessing (may not be available yet)
try
    using GlobtimPostProcessing
    const HAS_POSTPROCESSING = true
catch
    @warn "GlobtimPostProcessing not available - using basic analysis only"
    const HAS_POSTPROCESSING = false
end
...
for dim in sort(unique(df.dimension))
    ...
    recovery_stats = if nrow(df_dim_success) > 0
        ...
        if length(finite_errors) > 0
            @sprintf("mean=%.2e, median=%.2e",
                     mean(finite_errors),
                     median(finite_errors))
        else
            "all infinite"
        end
    else
...
if nrow(successful_results) > 0
    ...
    @printf "  Range: %d - %d\n" minimum(successful_results.critical_points) maximum(successful_results.critical_points)
    ...
    if "best_degree" in names(df)
        ...
        if length(degrees) > 0
            ...
            degree_counts = combine(groupby(DataFrame(degree=degrees), :degree), nrow => :count)
            sort!(degree_counts, :count, rev=true)
            for row in eachrow(degree_counts)
                @printf "    Degree %d: %d times (%.1f%%)\n" row.degree row.count (row.count / length(degrees) * 100)
            end
        end
    end
else
...
for row in eachrow(df)
    ...
    objective_str = isinf(row.objective_value) ? "Inf" : @sprintf("%.2e", row.objective_value)
...
if HAS_POSTPROCESSING
    ...
    for row in eachrow(df)
        ...
        try
            ...
            cp_files = filter(f -> startswith(f, "critical_points_deg_"), readdir(model_dir))
