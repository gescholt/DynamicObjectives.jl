...
display_parameters(param_names, hcat(p_true), labels = ["True Values"])
...
display_subsection("Running Globtim on 4D problem (this may take a while)...")

# Run standard experiment
t_start = time()
result = run_standard_experiment(
    objective_function = objective,
    objective_name = "lv4d_constrained",
    bounds = bounds,
    experiment_config = config,
    output_dir = output_dir,
    metadata = Dict{String,Any}("experiment_type" => "4D_test"),
    true_params = p_true,
)
...
if refined.n_converged > 0
    # Use progress bar for gradient computation (4D is slower)
    ...
    best_grad =
        ForwardDiff.gradient(objective, refined.refined_points[refined.best_refined_idx])
    display_results(["Best point ||∇f||" => norm(best_grad)], title = "Best Point Gradient")
else
...
# ==============================================================================
# Step 6: Solver Comparison — Tsit5 End-to-End Validation
# ==============================================================================

display_section("Step 6: Tsit5 End-to-End Validation")
display_subsection("Rerunning objective with Tsit5 solver to validate solver kwarg")

using Dynamic_objectives: Tsit5, Vern9

# Create objective with Tsit5 at coarser tolerance (faster for 4D)
objective_tsit5 = make_error_distance(
    model,
    outputs,
    ic,
    p_true,
...
# Verify it works at true params
obj_tsit5_at_true = objective_tsit5(p_true)
println("  Tsit5 objective at true params: $(round(obj_tsit5_at_true, digits=10))")
@assert obj_tsit5_at_true < 1e-4 "Tsit5 objective should be near zero at true parameters"

# Compare Tsit5 vs Vern9 at several points
println("\n  Comparing Tsit5(tol=1e-8) vs Vern9(tol=1e-10) at test points:")
comparison_points =
    [p_true, [1.1, 0.5, 1.0, 0.5], [1.0, 0.5, 1.1, 0.5], [0.8, 0.4, 0.8, 0.4]]

max_rel_diff = 0.0
for p in comparison_points
    v_tsit5 = objective_tsit5(p)
    v_vern9 = objective(p)
    denom = max(abs(v_vern9), 1e-15)
    rel_diff = abs(v_tsit5 - v_vern9) / denom
    max_rel_diff = max(max_rel_diff, rel_diff)
    println(
        "    p=$(round.(p, digits=1)): Tsit5=$(round(v_tsit5, sigdigits=6))  " *
        "Vern9=$(round(v_vern9, sigdigits=6))  rel_diff=$(round(rel_diff, sigdigits=2))",
    )
end

# Benchmark timing comparison
n_bench = 5
p_bench = [1.1, 0.5, 1.0, 0.5]

t_tsit5 = @elapsed for _ in 1:n_bench
    ;
    objective_tsit5(p_bench);
end
t_vern9 = @elapsed for _ in 1:n_bench
    ;
    objective(p_bench);
end
...
display_results(
    [
        "Max relative difference" => round(max_rel_diff, sigdigits = 2),
        "Tsit5 avg time" => "$(round(1000*t_tsit5/n_bench, digits=1)) ms",
        "Vern9 avg time" => "$(round(1000*t_vern9/n_bench, digits=1)) ms",
        "Speedup (Tsit5/Vern9)" => "$(round(t_vern9/t_tsit5, digits=2))x",
        "Conclusion" =>
            max_rel_diff < 0.01 ? "Tsit5 is viable for 4D screening" :
            "Use Vern9 for accuracy",
    ],
    title = "Solver Comparison: Tsit5 vs Vern9 (4D)",
)
