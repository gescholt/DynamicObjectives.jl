...
#
# Prerequisites: Run setup_dev_packages.jl once to install Globtim
...
display_section("Quick Start: Testing DynamicObjectives with globtim")
...
# ==============================================================================
# STEP 4: Run Globtim Optimization
# ==============================================================================

display_section("Step 4: Running Globtim optimization")
...
raw_result = run_standard_experiment(
    ...
    objective_name = "lv2d_quick_start",
    bounds = bounds,
...
refined_result =
    refine_experiment_results(raw_result[:output_dir], error_func, refinement_config)
...
# ==============================================================================
# STEP 6: Coarse vs Tight Tolerance Correctness Validation
# ==============================================================================

display_section("Step 6: Coarse vs Tight Tolerance Validation")
display_subsection("Verifying that coarse-tolerance objective produces consistent results")

using DynamicObjectives: Vern9, Tsit5

# Create objectives at different tolerances
error_func_coarse = make_error_distance(
    model,
    outputs,
    ic,
    p_true,
...
error_func_tight = make_error_distance(
    model,
    outputs,
    ic,
    p_true,
...
# Compare objective values at several test points
validation_points =
    [p_true, [1.1, 0.5], [1.0, 0.6], [0.5, 0.5], [2.0, 1.0], [0.8, 0.3], [1.5, 0.75]]

println("  Comparing objective values (coarse 1e-4 vs tight 1e-10):")
max_rel_diff = 0.0
for p in validation_points
    val_coarse = error_func_coarse(p)
    val_tight = error_func_tight(p)
    denom = max(abs(val_tight), 1e-15)
    rel_diff = abs(val_coarse - val_tight) / denom
    max_rel_diff = max(max_rel_diff, rel_diff)
    @printf(
        "    p=[%.1f,%.1f]: coarse=%.6e  tight=%.6e  rel_diff=%.2e\n",
        p[1],
        p[2],
        val_coarse,
        val_tight,
...
display_results(
    [
        "Max relative difference" => @sprintf("%.2e", max_rel_diff),
        "Points tested" => length(validation_points),
        "Status" => max_rel_diff < 0.01 ? "PASS (< 1% deviation)" : "WARN (> 1% deviation)",
    ],
    title = "Tolerance Validation Summary",
)

# Timing comparison
n_bench = 10
t_coarse = @elapsed for _ in 1:n_bench
    error_func_coarse([1.1, 0.6])
end
t_tight = @elapsed for _ in 1:n_bench
    error_func_tight([1.1, 0.6])
end

display_results(
    [
        "Coarse (1e-4) avg" => "$(round(1000*t_coarse/n_bench, digits=1)) ms",
        "Tight (1e-10) avg" => "$(round(1000*t_tight/n_bench, digits=1)) ms",
        "Speedup" => "$(round(t_tight/t_coarse, digits=1))x",
    ],
