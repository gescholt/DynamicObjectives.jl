...
function _find_results_root_from(start_dir::AbstractString)::String
    ...
    if haskey(ENV, "GLOBTIM_RESULTS_ROOT")
        ...
        isdir(root) || error(
            "GLOBTIM_RESULTS_ROOT is set but directory does not exist: $root")
        return root
    end
...
# Example
```julia
config = load_screening_config("Dynamic_objectives/examples/exp_candidates/lv4d.toml")
...
function load_screening_config(path::String)
    ...
    bounds = [Tuple{Float64,Float64}((Float64(pair[1]), Float64(pair[2])))
              for pair in mod["bounds"]]
...
No Globtim dependency required — this is purely Dynamic_objectives.
...
result = run_screening_from_config("Dynamic_objectives/examples/exp_candidates/lv4d.toml")
...
function run_screening_from_config(path::String; io::IO=stdout, verbose::Bool=true)
    ...
    println(io, "  Solver: $(config.solver_method) (abstol=$(config.solver_abstol), reltol=$(config.solver_reltol))")
    ...
    println(io)
    println(io, "─" ^ 100)
    println(io, "  screen_and_probe: $(config.n_candidates) candidates, $(config.n_probes) probes")
    println(io, "─" ^ 100)
    println(io)

    screen_start = time()
    screening_result = screen_and_probe(
        model, outputs, config.ic, config.bounds, config.time_interval;
    ...
    println(io)
    println(io, "─" ^ 100)
    println(io, "  Screening complete: $(length(ranked)) ranked / $(length(sweep.valid)) valid / $(sweep.diagnostics.n_total) total")
    ...
    for i in 1:n_show
        ...
        @printf(io, "    %d. p_true = [%s]  dynamic_range=%.2f  obj@true=%.2e\n",
            i,
    ...
    aggregate_fn = _resolve_function(config.aggregate_distances, AGGREGATION_REGISTRY, "aggregation")
