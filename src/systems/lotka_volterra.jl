...
function define_lotka_volterra_2D_sciml_benchmark()
    ...
    states = [x1, x2]
    @mtkcompile model = System(
        [D(x1) ~ a1 * x1 - 0.6 * x1 * x2,
            D(x2) ~ -a2 * x2 + 0.8 * x1 * x2],
        t,
        states,
        params
    )
...
function create_lv2d_localid1d_3d_objective(;
    ...
    E_lv2d = make_error_distance(
        lv2d_model, lv2d_outputs, lv2d_ic, lv2d_p_true,
        lv2d_time, lv2d_numpoints, L2_norm, sum;
    ...
    E_1d = make_error_distance(
        id1d_model, id1d_outputs, id1d_ic, id1d_p_true,
        id1d_time, id1d_numpoints, L2_norm, sum;
        return_inf_on_error = true
    )
