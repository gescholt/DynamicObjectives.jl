...
function score_landscape_grid(
    ...
    basin_frac = _compute_basin_fraction(grid_values, grid_axes, minima_indices, minima_points, p_true)

    # Deceptive minima
    deceptive_pts = _detect_deceptive_minima(
        minima_points, minima_values, p_true, bounds,
        deceptive_threshold, deceptive_distance,
    )
    ...
    return GridScoreResult(
        grid_values, grid_axes, grid_size,
        length(minima_indices), minima_indices, minima_values, minima_points,
        dyn_range, basin_frac,
        length(deceptive_pts), deceptive_pts,
        n_finite, n_total, elapsed_ms,
        dir_var, curv_score,
        depth_ratio, cond, rugged, plateau,
    )
end
    ...
    for idx in CartesianIndices(grid_values)
        ...
        for d in 1:ndim
            for offset in (-1, 1)
                ...
                neighbor_idx = CartesianIndex(
                    ntuple(k -> k == d ? neighbor_i : idx[k], ndim)
                )
...
"""
function _index_to_point(idx::CartesianIndex, grid_axes::Vector{Vector{Float64}})::Vector{Float64}
...
"""
function _compute_directional_variation(grid_values::Array{Float64})::Tuple{Vector{Float64}, Float64}
    ndim = ndims(grid_values)
    dims = size(grid_values)
    ...
    for d in 1:ndim
        ...
        for idx in CartesianIndices(grid_values)
            ...
            neighbor_idx = CartesianIndex(
                ntuple(k -> k == d ? idx[k] + 1 : idx[k], ndim)
            )
...
function _compute_conditioning(
    ...
    for d1 in 1:ndim
        ...
        for d2 in (d1+1):ndim
            ...
            idx_pp = CartesianIndex(ntuple(k -> k == d1 ? ci[k]+1 : k == d2 ? ci[k]+1 : ci[k], ndim))
            idx_pm = CartesianIndex(ntuple(k -> k == d1 ? ci[k]+1 : k == d2 ? ci[k]-1 : ci[k], ndim))
            idx_mp = CartesianIndex(ntuple(k -> k == d1 ? ci[k]-1 : k == d2 ? ci[k]+1 : ci[k], ndim))
            idx_mm = CartesianIndex(ntuple(k -> k == d1 ? ci[k]-1 : k == d2 ? ci[k]-1 : ci[k], ndim))
            ...
            (!isfinite(fpp) || !isfinite(fpm) || !isfinite(fmp) || !isfinite(fmm)) && return 0.0
...
function interestingness_score(gs::GridScoreResult)::Float64
    ...
    return 0.20 * multi + 0.10 * dyn + 0.12 * basin + 0.10 * decept + 0.13 * curv +
           0.12 * depth + 0.10 * cond_norm + 0.05 * rugged_norm + 0.08 * plateau_norm
end
    ...
    for i in 1:n
        ...
        try
            results[i] = score_landscape_grid(
                model, outputs, ic, p_true, bounds, time_interval;
                kwargs...,
            )
        catch e
            @warn "Grid scoring failed for candidate $i (index $candidate_idx)" exception=(e, catch_backtrace())
            results[i] = nothing
        end
    end
...
function print_grid_score(gs::GridScoreResult; io::IO = stdout)
    ...
    println(io, "  Finite evals:    $(gs.n_finite) / $(gs.n_total) ($(round(100*gs.n_finite/gs.n_total; digits=1))%)")
    ...
    println(io, "  Dynamic range:   $(round(gs.dynamic_range; digits=2)) orders of magnitude")
    println(io, "  Basin fraction:  $(round(gs.basin_fraction * 100; digits=1))% (p_true basin)")
    println(io, "  Deceptive min:   $(gs.n_deceptive)")
    println(io, "  Dir. variation:  [$(join([round(v; digits=3) for v in gs.directional_variation], ", "))]")
    println(io, "  Curvature score: $(round(gs.curvature_score; digits=3)) (1=curved, 0=flat)")
    println(io, "  Depth ratio:     $(round(gs.basin_depth_ratio; digits=3)) (1=equally deep competitor)")
    println(io, "  Conditioning:    $(round(gs.conditioning; digits=2)) (log10 condition number)")
    println(io, "  Ruggedness:      $(round(gs.ruggedness; digits=3)) (2nd/1st order ratio)")
