...
# Pretty and readable output for DynamicObjectives data structures
...
```
"""
function display_model_summary(
...
function display_parameters(
    ...
    # Select backend
    backend_map =
        Dict(:text => Val(:text), :ascii => Val(:ascii), :markdown => Val(:markdown))
    ...
    if config.use_color
        ...
        println()
        pretty_table(data, header = header, backend = backend, formatters = formatters)
        println()
    end
end
...
function display_time_series(
    ...
    # Display table with first/last few points (or all points if dataset is small)
    n_preview = min(5, length(t))
    show_ellipsis = length(t) > 2 * n_preview

    if show_ellipsis
        preview_indices = vcat(1:n_preview, (length(t)-n_preview+1):length(t))
        preview_indices = unique(sort(preview_indices))
    else
        preview_indices = collect(1:length(t))
    end
...
```
"""
function display_optimization_result(result; config::DisplayConfig = DEFAULT_CONFIG)
    # Extract fields
...
```
"""
function display_subsection(title::String; config::DisplayConfig = DEFAULT_CONFIG)
    if config.use_color
        println()
...
```
"""
function with_spinner(f::Function, message::String; config::DisplayConfig = DEFAULT_CONFIG)
