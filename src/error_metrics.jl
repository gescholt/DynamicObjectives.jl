        ...
        if eval_timeout !== nothing
            ...
            task = @async begin
                _compute_error_distance(
                    ...
                    data_sample_true, problem, model, ic,
...
function TolerantObjective(
    ...
    error_func = make_error_distance(
        ...
        distance_function, aggregate_distances, add_noise_in_time_series;
        return_inf_on_error=return_inf_on_error, eval_timeout=eval_timeout,
...
function _rebuild_error_func!(obj::TolerantObjective)
    ...
    obj._error_func = make_error_distance(
        a.model, a.outputs, a.ic, a.p_true,
        a.time_interval, a.numpoints,
        a.distance_function, a.aggregate_distances, a.add_noise_in_time_series;
        return_inf_on_error=a.return_inf_on_error, eval_timeout=a.eval_timeout,
        solver=obj.solver, abstol=obj.abstol, reltol=obj.reltol,
...
function resolve_aggregation(name::Symbol)
    haskey(AGGREGATION_STRATEGIES, name) ||
        throw(ArgumentError(
...
function make_partial_observability_distance(
    ...
    (minimum(output_indices) >= 1 && maximum(output_indices) <= length(outputs)) ||
        throw(ArgumentError(
    ...
    return make_error_distance(
        model, subset, ic, p_true, time_interval, args...; kwargs...
    )
end
