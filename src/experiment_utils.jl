...
function build_bounds(p_true, radii::AbstractVector{<:Real})
    length(radii) == length(p_true) || error("radii length ($(length(radii))) must match p_true length ($(length(p_true)))")
...
function print_candidate_summary_table(
    ...
    println("=" ^ 120)
    println("  $title")
    @printf("  Radius: %.4f, %d candidates, degrees %s\n",
        radius, length(entries), string(collect(degree_range)))
    println("=" ^ 120)
    println()

    # ── Full per-candidate, per-degree table ──
    println(
        rpad("#", 4), "  ", _pad("Name", name_width), "  ",
        lpad("Deg", 4), "  ", lpad("#CPs", 5), "  ",
        lpad("Raw f(x)", 11), "  ", lpad("Raw Recov", 12), "  ",
        lpad("L2 Error", 10), "  ", lpad("Rel L2", 10), "  ",
        lpad("Cap@1%", 8), "  ", lpad("Cap@5%", 8)
    )
    println("-" ^ 120)
    ...
    for r in all_results
        ...
        println(
            rpad(string(ci), 4), "  ", _pad(r.name, name_width), "  ",
            lpad(string(r.degree), 4), "  ", lpad(string(r.n_cps), 5), "  ",
            lpad(raw_str, 11), "  ", lpad(rec_str, 12), "  ",
            lpad(@sprintf("%.3e", r.l2_error), 10), "  ", lpad(rel_str, 10), "  ",
            lpad(c1_str, 8), "  ", lpad(c5_str, 8)
        )
    end
    ...
    println(
        rpad("#", 4), "  ", _pad("Name", name_width), "  ",
        lpad("Deg", 4), "  ", lpad("Raw Recov", 12), "  ",
        lpad("Raw f(x)", 12), "  ", lpad("Cap@1%", 8), "  ",
        lpad("Cap@5%", 8), "  ", "p_true"
    )
    println("-" ^ 120)
    ...
    for (ci, entry) in enumerate(entries)
        ...
        if isempty(rows)
            println(
                rpad(string(ci), 4), "  ", _pad(entry.name, name_width), "  ",
                lpad("---", 4), "  ", lpad("---", 12), "  ",
                lpad("---", 12), "  ", lpad("---", 8), "  ", lpad("---", 8)
            )
            continue
        end
        ...
        println(
            rpad(string(ci), 4), "  ", _pad(best.name, name_width), "  ",
            lpad(string(best.degree), 4), "  ", lpad(rec_str, 12), "  ",
            lpad(raw_str, 12), "  ", lpad(c1_str, 8), "  ",
            lpad(c5_str, 8), "  ", p_str
        )
    end
    ...
    if !isempty(best_per_candidate)
        ...
        if !isempty(recoveries)
            ...
            @printf("  Median raw recovery: %.3e\n", sort(recoveries)[div(length(recoveries) + 1, 2)])
            @printf("  Min raw recovery:    %.3e\n", minimum(recoveries))
            @printf("  Max raw recovery:    %.3e\n", maximum(recoveries))

            cap5_rates = [r.capture_5pct for r in best_per_candidate if !isnan(r.capture_5pct)]
            if !isempty(cap5_rates)
                n_full_cap = count(r -> r == 1.0, cap5_rates)
                @printf("  Cap@5%% = 100%%:       %d/%d candidates\n", n_full_cap, length(cap5_rates))
            end

            cap1_rates = [r.capture_1pct for r in best_per_candidate if !isnan(r.capture_1pct)]
            if !isempty(cap1_rates)
                n_full_cap1 = count(r -> r == 1.0, cap1_rates)
                @printf("  Cap@1%% = 100%%:       %d/%d candidates\n", n_full_cap1, length(cap1_rates))
            end
        end
    end
