    ...
    try
        solution = Logging.with_logger(Logging.NullLogger()) do
            ModelingToolkit.solve(
                ...
                verbose=false, maxiters=1_000_000
            )
        end
    catch e
...
function _generate_candidates(
    ...
    if sampling == :random
        return [
            [lo[d] + rand() * (hi[d] - lo[d]) for d in 1:dim]
            for _ in 1:n
        ]
...
function sweep_p_true(
    ...
    for (i, p) in enumerate(candidates)
        result = is_trajectory_bounded(
            model, ic, p, time_interval;
            threshold=threshold, numpoints=numpoints,
        ...
        if verbose && (i % max(1, n_candidates ÷ 10) == 0 || i == length(candidates))
            ...
            println("  [$n_done/$(length(candidates))] " *
    ...
    if verbose
        println()
        println("  Sweep complete: $n_valid/$n_total passed ($(round(100 * n_valid/n_total, digits=1))%)")
        println("  Rejected: $n_solver_fail solver failures, $n_amplitude amplitude exceeded")
        println("  Time: $(round(elapsed, digits=2))s ($(round(diagnostics.ms_per_candidate, digits=1)) ms/candidate)")
        if !isempty(amplitudes)
            println("  Amplitude range: $(round(diagnostics.amplitude_min, digits=2)) — $(round(diagnostics.amplitude_max, digits=2))")
        end
    end
...
function probe_landscape(
    ...
    dim = length(bounds)
    probe_points = [
        [bounds[d][1] + rand() * (bounds[d][2] - bounds[d][1]) for d in 1:dim]
        for _ in 1:n_probes
    ]
...
function rank_probes(
    ...
    sort!(ranked_indices, by = i -> begin
...
function screen_and_probe(
    ...
    sweep = sweep_p_true(
        model, ic, bounds, time_interval;
        n_candidates=n_candidates, margin=margin, sampling=sampling,
        threshold=threshold, numpoints=numpoints_screen,
        solver=solver, abstol=abstol, reltol=reltol,
        verbose=verbose
    )
    ...
    for (i, p) in enumerate(sweep.valid)
        probes[i] = probe_landscape(
            model, outputs, ic, p, bounds, time_interval;
            n_probes=n_probes, numpoints=numpoints_probe,
    ...
    if verbose
        ...
        if n_ranked > 0
            ...
            println("  Probe time: $(round(t_probe_total, digits=2))s " *
                    "($(round(t_probe_total/n_valid*1000, digits=1)) ms/candidate)")
        end
        println("  Total time: $(round(sweep.diagnostics.elapsed_seconds + t_probe_total, digits=2))s")
    end
