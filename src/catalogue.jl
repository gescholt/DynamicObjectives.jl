...
function _function_name(fn::Function, registry::Dict{String,Function}, kind::String)
    ...
    error("$kind function is not registered for serialization. " *
...
function _init_registries!()
    ...
    register_model!("define_daisy_ex3_model_4D_no_input", define_daisy_ex3_model_4D_no_input)
    register_model!("define_generalized_lotka_volterra_4D", define_generalized_lotka_volterra_4D)
    register_model!("define_constrained_lotka_volterra_4D", define_constrained_lotka_volterra_4D)
    ...
    register_model!("define_lotka_volterra_3D_model_locally_identifiable", define_lotka_volterra_3D_model_locally_identifiable)
    ...
    register_model!("define_lotka_volterra_2D_model_v3", define_lotka_volterra_2D_model_v3)
    register_model!("define_lotka_volterra_2D_model_v3_two_outputs", define_lotka_volterra_2D_model_v3_two_outputs)
    register_model!("define_lotka_volterra_2D_sciml_benchmark", define_lotka_volterra_2D_sciml_benchmark)
    register_model!("define_fitzhugh_nagumo_3D_model", define_fitzhugh_nagumo_3D_model)
    register_model!("define_fitzhugh_nagumo_3D_model_two_outputs", define_fitzhugh_nagumo_3D_model_two_outputs)
    register_model!("define_goodwin_oscillator_4D", define_goodwin_oscillator_4D)
    register_model!("define_simple_2D_model_locally_identifiable", define_simple_2D_model_locally_identifiable)
    register_model!("define_simple_2D_model_locally_identifiable_square", define_simple_2D_model_locally_identifiable_square)
    register_model!("define_simple_1D_model_locally_identifiable", define_simple_1D_model_locally_identifiable)
    ...
    register_model!("define_rosenzweig_macarthur_3d_model", define_rosenzweig_macarthur_3d_model)
    register_model!("define_goodwin_3d_product_obs_model", define_goodwin_3d_product_obs_model)
    register_model!("define_goodwin_3d_locally_id_model", define_goodwin_3d_locally_id_model)
...
function create_objective(entry::CatalogueEntry)
    ...
    return make_error_distance(
        ...
        entry.time_interval, entry.numpoints,
...
```
"""
function create_tolerant_objective(entry::CatalogueEntry;
                                   abstol::Real=1e-10,
                                   reltol::Real=1e-10,
                                   solver=Vern9())
    model, _, _, outputs = entry.model_fn()
...
function create_refinement_objective(entry::CatalogueEntry)
    ...
    return make_error_distance(
        model, outputs, entry.ic, entry.p_true,
        entry.time_interval, entry.numpoints,
        dist_fn, entry.aggregate_distances;
        return_inf_on_error = true,
        eval_timeout = entry.eval_timeout
    )
end
...
function _entry_to_dict(entry::CatalogueEntry)
    d = Dict{String,Any}(
        ...
        "distance_function" => _function_name(entry.distance_function, DISTANCE_REGISTRY, "distance"),
        "aggregate_distances" => _function_name(entry.aggregate_distances, AGGREGATION_REGISTRY, "aggregation"),
        "eval_timeout" => entry.eval_timeout,
    )
    if entry.refinement_distance_function !== nothing
        d["refinement_distance_function"] = _function_name(entry.refinement_distance_function, DISTANCE_REGISTRY, "distance")
    end
    return d
end
...
function _dict_to_entry(d::Dict)
    dist_fn = _resolve_function(String(d["distance_function"]), DISTANCE_REGISTRY, "distance")

    # Resolve refinement_distance_function: explicit in dict → auto-detect from log_ prefix → nothing
    ref_dist = if haskey(d, "refinement_distance_function") && d["refinement_distance_function"] !== nothing
        _resolve_function(String(d["refinement_distance_function"]), DISTANCE_REGISTRY, "distance")
    else
    ...
    return CatalogueEntry(
        ...
        ic = Float64.(d["ic"]),
        bounds = [Tuple{Float64,Float64}((Float64(pair[1]), Float64(pair[2]))) for pair in d["bounds"]],
        time_interval = Float64.(d["time_interval"]),
        numpoints = Int(d["numpoints"]),
        distance_function = dist_fn,
        aggregate_distances = _resolve_function(String(d["aggregate_distances"]), AGGREGATION_REGISTRY, "aggregation"),
